...
pub async fn upload_to_cloud(
    ...
    let task_id = task_id.unwrap_or_else(|| format!("upload_{}", std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_millis())
        .unwrap_or(0)));
    
    // 并行上传到所有配置的云存储
    let upload_futures: Vec<_> = configs.into_iter().map(|config| {
        ...
        tokio::spawn(async move {
            ...
            let result = match config.provider.as_str() {
                "google_drive" => upload_to_google_drive_resumable(&file_path_clone, &config, &app_clone, &task_id_clone).await,
                _ => Err(format!("不支持的云存储提供商: {}", config.provider)),
            };
            
            match &result {
                Ok(file_id) => {
                    info!("成功上传到 {} ({})，文件ID: {}", config.name, config.provider, file_id);
                }
                Err(e) => {
...
async fn upload_to_google_drive_resumable(
    ...
    let file_size = path.metadata()
        .map(|m| m.len())
        .unwrap_or(0);
    info!("文件大小: {} 字节 ({:.2} MB)", file_size, file_size as f64 / 1024.0 / 1024.0);
    ...
    // 发送初始进度 0%
    let _ = app.emit("upload-progress", UploadProgressEvent {
    ...
    while uploaded < file_size {
        ...
        let response = client
            ...
            .header("Content-Range", format!("bytes {}-{}/{}", start_byte, end_byte, file_size))
            .body(buffer)
            .send()
            .await
            .map_err(|e| {
